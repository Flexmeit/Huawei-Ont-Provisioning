...
hostname = "172.16.248.15"
username = "py.script"
password = "Almyaa2025"
port = 22

# Dynamic Slot and Port
slot = str(random.choice([0, 1]))  # Slot Dynamic, 0 - 1
port_number = "14"  # Port
vlan = "81"  # VLAN
...
def process_ont(sn, port):
    ...
    time.sleep(0.5)
    connection.send(f"ont confirm {port} sn-auth {sn} omci ont-lineprofile-id 10 ont-srvprofile-id 1 desc 'python_registering'\n")
    time.sleep(2)
    connection.send('\n')
    time.sleep(1)
